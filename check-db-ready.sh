...
# mongo ping test
until echo '\q' | docker exec logger_db mongo --host ${MONGO_HOST} ${MONGO_INITDB_DATABASE} -u${MONGO_USER_NAME} -p${MONGO_USER_PASSWORD} --authenticationDatabase ${MONGO_INITDB_ROOT_USERNAME}> /dev/null 2>&1; do
    if [ $CURRENT_TRY -eq $MAX_TRY ]; then
